...
updates:
    added option to run nuclear flexibly or at capacity
    cleaned up code and comments
    
to-dos:
    confirm length of time simulated
    confirm units of costs, capacity, charge/discharge, etc. - see notes
    curtailment: total vs. individual technology? - currently calculating total
...
def core_model(
    ...
    # input data: demand, solar and wind capacity factor        
    ...
    # generation
        ...
        # dispatch_[generation] = energy generated at all timesteps (timestep size = 1hr) from each generator [kWh]
    
    # curtailment
        # dispatch_curtailment = curtailed renewable energy generation at all timesteps [kWh]
        # note: this is more like a dummy variable in the current model
    ...
    if 'solar' in system_components:
        ...
        # if solar not in system, set capacity and dispatch to zero
        capacity_solar = 0
    ...
    if 'wind' in system_components:
        ...
        # if wind not in system, set capacity and dispatch to zero
        capacity_wind = 0
    ...
    if 'nuclear' in system_components:
        ...
        constraints += [
                capacity_nuclear >= 0,
                dispatch_nuclear >= 0, 
                # scenario: "rampable" nuclear
#                dispatch_nuclear <= capacity_nuclear 
                # scenario: "baseload" nuclear
                dispatch_nuclear == capacity_nuclear
                ]
        ...
        # if nuclear not in system, set capacity and dispatch to zero
        capacity_nuclear = 0
...
#        constraints += [energy_storage[0] == 0.0]
    ...
    # supply = demand at all timesteps
    constraints += [

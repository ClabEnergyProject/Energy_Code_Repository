...
updates:
    removed extra ".csv" from filenames
    cleaned up code and comments

to-dos:
    vary baseline fixed and variable costs - currently using EIA's assumptions - see Fan's notes "generator and storage LCOE" - other sources?
    read "original" cost and efficiency data as input from .csv files (instead of "hardwiring" assumptions)?
    confirm units of costs used in script and output files - see notes
    normalize wind and solar capacity to demand
...
#    sa_var_cost_nuclear[::-1].sort()
    ...
    if case_switch == 'nuc_vs_renew_ng':
        system_components = ['natural_gas','wind','solar','nuclear','storage']
        fix_cost_nuclear  = sa_fix_cost_nuclear * fix_cost_nuclear
#        var_cost_nuclear = sa_var_cost_nuclear * var_cost_nuclear
    elif case_switch == 'nuc_vs_renew_ngccs':
        system_components = ['natural_gas','wind','solar','nuclear','storage']
        fix_cost_nuclear  = sa_fix_cost_nuclear * fix_cost_nuclear
#        var_cost_nuclear = sa_var_cost_nuclear * var_cost_nuclear
        fix_cost_natgas = 2 * fix_cost_natgas
        var_cost_natgas = 2 * var_cost_natgas
    elif case_switch == 'nuc_vs_renew_no_ng':
